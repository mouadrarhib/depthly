...
interface MonthlyViewProps extends AnalyticsProjectScope {
  date: Date
}
...
export function MonthlyView({ date, projectId }: MonthlyViewProps) {
  ...
  // range. It is enabled here only when a project scope needs raw session totals.
  const isProjectFiltered = projectId !== undefined
  const { data: monthSessions, isLoading: loadingSessions } = useSessionsForWeek(
    firstKey,
    lastKey,
    projectId,
    isProjectFiltered
  )
  const { data: prevMonthProjectSessions, isLoading: loadingPrevSessions } = useSessionsForWeek(
    prevFirst,
    prevLast,
    projectId,
    isProjectFiltered
  ...
  const isLoading =
    loadingThis || loadingPrev || (isProjectFiltered && (loadingSessions || loadingPrevSessions))
  ...
  const summaryMap = isProjectFiltered
    ? sessionsToDailyTotals(monthSessions ?? [])
    : new Map((summaries ?? []).map((s) => [s.date, s]))
  const prevMap = isProjectFiltered
    ? sessionsToDailyTotals(prevMonthProjectSessions ?? [])
    : new Map((prevSummaries ?? []).map((s) => [s.date, s]))

  const thisMonthMinutes = days.reduce(
    (sum, d) => sum + (summaryMap.get(formatPeriodKey(d, 'daily'))?.focus_minutes ?? 0),
    0
  )
  const prevMonthMinutes = prevDays.reduce(
    (sum, d) => sum + (prevMap.get(formatPeriodKey(d, 'daily'))?.focus_minutes ?? 0),
    0
  )
  const thisMonthSessions = days.reduce(
    (sum, d) => sum + (summaryMap.get(formatPeriodKey(d, 'daily'))?.session_count ?? 0),
    0
  )
  const prevMonthSessions = prevDays.reduce(
    (sum, d) => sum + (prevMap.get(formatPeriodKey(d, 'daily'))?.session_count ?? 0),
    0
  )
          ...
          <>
            <div style={cardStyle}>
              ...
              {prevMonthMinutes > 0 &&
                (() => {
                  const trendColor =
                    thisMonthMinutes > prevMonthMinutes
                      ? '#3DD68C'
                      : thisMonthMinutes < prevMonthMinutes
                        ? '#7A7890'
                        : '#7A7890'
                  ...
                  return (
                    ...
                    >
                      {trendArrow}{' '}
                      {formatMinutesToHours(Math.abs(thisMonthMinutes - prevMonthMinutes))} vs last
                      month
                    </span>
                  )
                })()}
          ...
          >
            <div style={{ width: '100%' }}>
              ...
              <TooltipProvider delayDuration={100}>
                ...
                >
                  ...
                  {days.map((d) => {
                    ...
                    const dayProjects = [...dayProjectMap.values()].sort(
                        ...
                        <TooltipTrigger asChild>
                          <div
                            style={{
                              ...
                              boxShadow: isToday ? '0 0 0 2px #4B9EFF, 0 0 0 4px #141417' : 'none',
                            }}
                          >
                            <span
                              style={{
                                fontSize: 11,
                                color: isFuture ? '#5A5868' : numColor,
                                lineHeight: 1,
                              }}
                            >
                              {dayNum}
                        ...
                        <TooltipContent side="top">
                          <div style={{ minWidth: 140 }}>
                            <div className="font-data" style={{ fontSize: 12, color: '#E8E6F0' }}>
                              {monthLabel}
                              {minutes > 0 && ` — ${formatMinutesToHours(minutes)}`}
                            </div>
                            {minutes === 0 && (
                              ...
                              >
                                {dayProjects.map((p) => (
                                  ...
                                  >
                                    ...
                                    <span
                                      className="font-data"
                                      style={{ fontSize: 11, color: '#E8E6F0', flexShrink: 0 }}
                                    >
              ...
              >
                ...
                {[
                  '#1A1A1F',
                  'rgba(75,158,255,0.18)',
                  'rgba(75,158,255,0.35)',
                  'rgba(75,158,255,0.55)',
                  '#4B9EFF',
                ].map((color, i) => (
                  <div
                    key={i}
