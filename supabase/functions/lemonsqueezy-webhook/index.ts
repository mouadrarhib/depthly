...
async function upsertSubscription(
  ...
  if (profileError) {
    // profiles.stripe_customer_id and stripe_subscription_id are both UNIQUE.
    // Lemon Squeezy (especially in test mode) can reuse the same underlying
    // customer record across checkouts started from different app accounts,
    // which trips this constraint here even though the event is legitimate.
    // These two columns are a denormalized display cache only (nothing in the
    // app reads them — BillingPage and the rest of the billing UI read the
    // `subscriptions` table and profiles.plan, per CLAUDE.md's "profiles.plan
    // is the fast-read billing state"), so on a unique violation, retry without
    // them rather than losing the plan/status update that actually gates access.
    if (profileError.code === '23505') {
      console.warn('Unique constraint hit on stripe_customer_id/stripe_subscription_id — retrying profile update without them:', profileError.message)
      ...
      if (retryError) {
        console.error('Failed to update profile for subscription event (retry without LS ids):', retryError)
        throw retryError
      }
      return
    }
