-- ============================================================================
-- MIGRATION 002 — save_session RPC
-- ============================================================================
-- Run in: Supabase Dashboard → SQL Editor → New Query → Run
--
-- What this does:
--   Creates the save_session() SECURITY DEFINER function that atomically:
--     1. Inserts into sessions
--     2. Upserts daily_summaries (focus_minutes + session_count)
--     3. Marks daily_goal_met if the user's daily goal was reached
--     4. Upserts user_stats for daily / weekly / monthly / yearly periods
--     5. Updates profiles (total_focus_minutes, total_sessions, streak)
--     6. Increments tasks.actual_pomodoros if a task was linked
--
--   Only focus sessions update aggregates — break sessions are stored but
--   don't count toward streak, stats, or goal progress.
--
--   'custom' timer_mode is coerced to 'pomodoro' for storage because the
--   existing timer_mode_type enum only has ('pomodoro', 'free').
-- ============================================================================
...
declare
  ...
  v_today          date := (p_started_at at time zone 'UTC')::date;
