...
The Analytics page (`/analytics`) gives users a retrospective view of their
focus sessions through a lifetime Overview and four time granularities:
**daily**, **weekly**, **monthly**, and **yearly**. Every view can be scoped to
all projects, one active or archived project, or sessions with no project.
...
- **`sessions`** (raw) — queried by every view for project-aware totals,
  ...
  join, with an optional project filter applied in Supabase.
...
`AnalyticsPage` owns a URL-backed project scope through the `project` search
parameter. With no parameter the page shows `All projects`; `project=unassigned`
shows sessions whose `project_id` is null; any other value is treated as a
project UUID. The selector includes both active and archived projects so
historical analytics do not disappear when a project is archived.

The filter remains active while switching tabs or navigating dates. A compact
scope label is rendered inside `analyticsCaptureRef`, so Share Progress images
identify the selected project even though the interactive selector itself is
outside the captured content. Filtered filenames also include a project slug.

`AnalyticsProjectScope` (`src/types/app.ts`) carries `projectId`, label, and
color into every view. `projectId` has three deliberate states:

- `undefined`: all projects
- `null`: sessions without a project
- `string`: one project UUID

Daily and weekly goals remain account-wide because goals are not stored per
project. When a project filter is active, their cards are labeled **Overall
Daily Goal** and **Overall Weekly Goal** and continue using unfiltered
`daily_summaries`. All other visible focus/session metrics use the selected
project scope.
...
<AnalyticsProjectSelect />  ← always visible; stacks full-width on mobile
<ShareProgressButton />     ← period tabs only
...
Receives `AnalyticsProjectScope` and reads data internally via `useProfile()`
and `useSessionsAllTime(projectId)`.
...
- Data comes from `useSessionsAllTime(projectId)`, which fetches raw sessions
  with `started_at` and aggregates them client-side. In filtered mode the same
  rows also drive total focus, session count, current/longest project activity
  streaks, average per day, and first-session date.
...
Renders: `‹ [calendar icon + period label] ›` with an optional "Today / This week / This month / This year" jump link when not on the current period.

- Uses `getPeriodLabel(currentDate, period)` for the center label
- The center label is a button that opens a native date picker. Selecting a date jumps directly to that day; Weekly, Monthly, and Yearly views use the week, month, or year containing the selected date.
- The picker is capped at today, matching the disabled next-arrow behavior and preventing navigation into future analytics periods.
...
- `WeeklyView` — `pieData` built from `useSessionsForWeek(mondayKey, sundayKey)`.
...
- Rendered via the shared `ProjectBreakdownCard` component using raw focus sessions from `useSessionsForWeek`.
- In "All projects" scope the donut shows the per-project split. In a specific-project or "No project" scope it becomes a readable single-scope total while retaining the same card layout.
...
- Fetches account-wide current-week summaries for the Overall Weekly Goal.
- Fetches raw current-week focus sessions for the selected scope. A previous-week raw-session query is enabled only when a project filter is active; unfiltered comparisons continue using the existing summary rows.
...
date (Date)
  ...
  → useDailySummariesRange(mon, sun)     → overall goal progress
  → useSessionsForWeek(mon, sun, scope)  → filtered focus sessions
  + previous period source               → filtered sessions or unfiltered summaries

session totals are grouped by local calendar date into chartData[7].
thisWeekMinutes: reduce over the selected scope's daily totals.
```
...
date (Date)
  ...
  → useSessionsForWeek(first, last, scope) → filtered focus sessions for the month
  + same for previous month                → filtered comparison

Calendar renders days array, looking up each key in the local-date session totals map.
...
date (Date)
  ...
  → useSessionsForYear(year, scope)      → filtered raw focus sessions for the year

focusMap: Map<local-date-string, focus_minutes>
...
### Donut — project breakdown (DailyView, WeeklyView, YearlyView, OverviewView)
...
#### `useSessionsForDay(date, projectId?)`

```ts
function useSessionsForDay(
  date: string,
  projectId?: string | null
): UseQueryResult<SessionWithProject[]>
```

The query key includes a normalized project scope (`all`, `unassigned`, or the
project UUID). Fetches raw focus sessions for the local calendar day, joined
with `projects(name, color)`.

#### `useSessionsForYear(year, projectId?)`

```ts
function useSessionsForYear(
  year: number,
  projectId?: string | null
): UseQueryResult<SessionProjectSliceWithDate[]>
```

Fetches minimal raw session fields including `started_at`. YearlyView converts
filtered sessions into daily totals for its stats and heatmap.

#### `useSessionsForWeek(start, end, projectId?, enabled?)`

Despite its historical name, this hook accepts any inclusive local-date range.
Weekly and Monthly views use it for current-period sessions and, while filtered,
for previous-period comparisons. The optional `enabled` flag avoids fetching an
extra previous-period session range in the unfiltered state.

#### `useSessionsAllTime(projectId?)`

```ts
function useSessionsAllTime(
  projectId?: string | null
): UseQueryResult<SessionProjectSliceWithDate[]>
```

Fetches every matching focus session with `started_at`; the query key includes
the normalized project scope.

#### `useUserStats(periodType, periodKey)`

```ts
function useUserStats(periodType: string, periodKey: string): UseQueryResult<UserStats | null>
```
...
```ts
function useUserStatsRange(periodType: string, periodKeys: string[]): UseQueryResult<UserStats[]>
```
...
#### `fetchSessionsForDay(userId, date, projectId?)`
...
#### `fetchSessionsForYear(userId, year, projectId?)`
...
type SessionProjectSliceWithDate = SessionProjectSlice & {
  started_at: string
}
```

Selects only the columns needed for project and local-date aggregation. Filters `type = 'focus'`. No ordering applied (caller aggregates, doesn't render chronologically).

#### `fetchSessionsForWeek(userId, startDate, endDate, projectId?)`

Fetches raw focus sessions across an inclusive local-calendar date range. WeeklyView uses a seven-day range; MonthlyView reuses it for the full current and previous months.

#### `fetchSessionsAllTime(userId, projectId?)`

```ts
async function fetchSessionsAllTime(
  userId: string,
  projectId?: string | null
): Promise<SessionProjectSliceWithDate[]>
```
...
```ts
function formatPeriodKey(date: Date, period: 'daily' | 'weekly' | 'monthly' | 'yearly'): string
```
...
```ts
function getBestHourOfDay(sessions: Array<{ started_at: string; duration_mins: number }>): string
...
```ts
function getBestDayOfWeek(summaries: Array<{ date: string; focus_minutes: number }>): string
